...
package body Quote_Structure is
   ...
   function To_String (Source : Unbounded_String) return String renames
   ...
   function New_Quote return Quote is
     (Value =>
      ...
      Has   =>
        (Speaker   => False, Source => False, Author => False,
         Quotation => True));
   ...
   begin
      for Quote of Quotes loop
         declare
            Node : JSON_Value := Create_Object;
         begin
            for Field in Fields loop
               if Quote.Has (Field) then
                  Node.Set_Field
                    (To_String (Field_Keys (Field)), Quote.Value (Field));
               end if;
            end loop;
            Append (Data, Node);
         end;
   ...
   function Quote_Item
     (From : Quote_Vector'Class; Index : Natural) return Quote is
